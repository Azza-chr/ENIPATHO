...
export class MessagesPageComponent implements OnInit, OnDestroy {
  ...
  get departmentContacts(): DepartmentContact[] {
    ...
    const fromMessages = this.allMessages
      .filter((m) => m.fromRole === 'etudiant' || m.toRole === 'etudiant')
      .map((m) => ({
        key: m.contactKey ?? this.contactKeyForName(m.contactName || m.fromName, 'etudiant'),
        name: m.contactName || (m.fromRole === 'etudiant' ? m.fromName : 'Etudiant'),
        subtitle: m.subject || 'Conversation',
    ...
    const contacts = [...base, ...fromMessages].reduce(
      (map, contact) => {
        ...
        if (!current || ((contact as any).latestAt ?? '') > (current.latestAt ?? '')) {
          map.set(contact.key, { ...current, ...contact, latestAt: (contact as any).latestAt ?? current?.latestAt ?? '' });
        }
        return map;
      },
    ...
    return Array.from(contacts.values())
      .map((contact) => {
        const related = this.allMessages.filter(
          (m) => (m.contactKey ?? this.contactKeyForName(m.contactName || m.fromName, 'etudiant')) === contact.key
        );
        const latestAt = related.reduce(
        ...
        return {
          ...
          unread: related.some((m) => m.fromRole === 'etudiant')
        };
      })
      .sort((a, b) => {
        if (!a.latestAt && !b.latestAt) return a.name.localeCompare(b.name);
  ...
  get activeContact(): DepartmentContact {
    ...
    return this.departmentContacts.find((c) => c.key === this.selectedContactKey) ?? this.fallbackContact();
  }
  ...
  ngOnInit(): void {
    ...
    this.subscription = this.communication.messages$.subscribe(() => this.refresh());
  }
  ...
  send(): void {
    const session = this.session;
    if (!session || !this.form.get('body')?.value) return;
  ...
  private refresh(): void {
    ...
    if (this.session.role === 'chef-departement') {
      const hasContact = this.departmentContacts.some((c) => c.key === this.selectedContactKey);
      if (!hasContact) {
  ...
  private contactKeyForName(value: string, prefix = 'enseignant'): string {
    return `${prefix}-${value.toLowerCase().normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || 'general'}`;
  }
}