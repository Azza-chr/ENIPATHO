...
export class AuthService {
  ...
  login(credentials: { email: string; password: string }) {
    return this.http.post<any>(
      ...
      tap(response => {
        const role = this.mapRole(response.role);
        this.updateSession({
          token: response.token,
          role: role,
          email: response.userEmail,
          // ✅ Utilise le vrai nom depuis le backend
          fullName: (response.userNom && response.userPrenom)
            ? `${response.userNom} ${response.userPrenom}`
            : response.userEmail,
  ...
  private mapRole(role: string): AppRole {
    const map: Record<string, AppRole> = {
      'ADMIN': 'chef-departement',
      'ENSEIGNANT': 'enseignant',
      'ETUDIANT': 'etudiant'
    };
    return map[role.toUpperCase()] ?? 'etudiant';
  }
  ...
  private restoreSession(): AuthSession | null {
    ...
    try {
      ...
      if (!parsed.role || !parsed.email || !parsed.fullName || !parsed.redirectTo) return null;
