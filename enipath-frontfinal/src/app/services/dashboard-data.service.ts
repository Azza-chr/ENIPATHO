...
export class DashboardDataService {
      ...
      map(({ coursByMatiere }) => {
        ...
        return {
          stats: [
            ...
            { label: 'Groupes affectes', value: '-', detail: 'Endpoint statistiques enseignant a brancher' },
  ...
  private toNotifications(value: unknown): DashboardNotification[] {
    return this.extractArray(value).map((source) => ({
      ...
      link: this.toOptionalText(source['link'])
    }));
  }
  ...
  // ✅ CORRECTION : toCourseRecommendations avec link vers la matière
      ...
      .flatMap((item) =>
        item.cours
          ...
          .map((cours) => ({
            ...
            link: `/etudiant/matieres/${item.matiere.id}`
          }))
      )
      .slice(0, 4);
  }
  ...
  private average(values: number[]): number {
    ...
    if (usefulValues.length === 0) return 0;
