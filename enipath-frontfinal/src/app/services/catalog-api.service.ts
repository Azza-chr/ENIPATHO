...
export class CatalogApiService {
  ...
  // ✅ Récupère toutes les matières (pour le sélecteur enseignant)
  getMatieres(): Observable<Matiere[]> {
    return this.http.get<unknown>(`${this.apiUrl}/matieres`).pipe(
      map((payload) => this.extractArray(payload).map((item) => this.toMatiere(item)))
    );
  }
    ...
    return this.http.get<unknown>(`${this.apiUrl}/groupes`).pipe(
      map((payload) =>
        this.extractArray(payload).map((item) => ({
          ...
          nom: this.toText(item['libelle'] ?? item['nom'], 'Groupe'),
