...
import { CoursPdf, Groupe } from '../../models/catalog.models';
...
export class TeacherPageComponent implements OnInit {
  ...
  async publishCourse(): Promise<void> {
    ...
    try {
      ...
      this.setFeedback('success', 'Le cours PDF a bien ete partage au groupe choisi. Il doit maintenant apparaitre cote etudiant.');
    } catch (error) {
  ...
  private extractNumericId(payload: unknown): number {
    // First, extract the actual data object (similar to extractItem pattern)
    ...
    // Check for nested object wrappers (like extractItem does)
    const nested = [source['data'], source['item'], source['result'], source['value']].find(
      (candidate) => candidate && typeof candidate === 'object' && !Array.isArray(candidate)
    ) as Record<string, unknown> | undefined;

    // Use nested object if found, otherwise use source
    const dataObject = nested ?? source;

    // Try common ID field names in order
    const id = 
      dataObject['id'] ?? 
      dataObject['courseId'] ?? 
      dataObject['coursId'] ?? 
